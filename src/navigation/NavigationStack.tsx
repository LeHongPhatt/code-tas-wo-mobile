...
const StyledTabBarIcon = ({route, color, isFocused}) => {
  const iconStyles = {
    borderTopWidth: isFocused ? 2 : 0,
    borderColor: Colors.white,
    paddingTop: 2,
  };
  return <View style={iconStyles}>{Icon[`${route?.name}`]({color})}</View>;
};
const HomeTabs = () => {
    ...
    <Tab.Navigator
      screenOptions={({route}) => ({
        ...
        tabBarIcon: ({color, focused}) => (
          <StyledTabBarIcon route={route} color={color} isFocused={focused} />
        ),
